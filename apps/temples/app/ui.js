...
function statCards(stats = {}, locale = "en") {
  ...
  if (stats.age_years) {
    cards.push({ label: t(locale, "approxAge"), value: `${stats.age_years}+ ${t(locale, "years")}` });
  }
  if (stats.current_temple_built) {
    cards.push({ label: t(locale, "currentTemple"), value: `${stats.current_temple_built}` });
  }
  if (stats.daily_visitors) {
    cards.push({ label: t(locale, "dailyVisitors"), value: stats.daily_visitors });
  }
  if (stats.reconstructions) {
    cards.push({ label: t(locale, "rebuildCycles"), value: `${stats.reconstructions}` });
  }
  ...
  return `
    <section class="planner-card" aria-label="${escapeHtml(t(locale, "plannerTitle"))}">
      <div>
        ...
        <h2>${escapeHtml(t(locale, "progress"))}: ${planner.visited}/${planner.total} (${progressPercent}%)</h2>
        <p>${escapeHtml(t(locale, "nextTemple"))}: ${escapeHtml(nextTempleName)}</p>
      </div>
...
export function renderHomeView({
  ...
  const body = (() => {
    if (loading) {
      return `<section class="state-card"><p>${escapeHtml(t(locale, "loadingCatalog"))}</p></section>`;
    }
    if (error) {
    ...
    return temples
      .map((temple, index) => {
        ...
        return `
        <article class="temple-card" data-action="open-temple" data-id="${escapeHtml(temple.id)}" aria-label="${escapeHtml(templeDisplayName(temple, locale))}">
...
export function renderTempleView({ temple, bundle, loading, error, locale = "en", isSaved = false, isVisited = false }) {
  if (loading) {
    return `
      <section class="state-card"><p>${escapeHtml(t(locale, "loadingTemple"))}</p></section>
    `;
  }
